...
# the vlc.MediaPlayer can't free memory automatically, must use only one instance。
...
class ShellSoundPlayer(AbstractSoundPlayer):
    ...
    def run(self):
        ...
        self.pipe = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.playing = True
        self.pipe.wait()
        self.playing = False
...
def get_sound_manager(audio=None):
    ...
    if not _sound_instance:
        # _sound_instance = Sound(config.get('sound_engine', 'pyaudio'), audio=audio)
        _sound_instance = Sound('pyaudio', audio=audio)
...
if __name__ == '__main__':
    ...
    sound.play('/home/robot/aliada-car-robot/static/audio/beep_hi.wav')
