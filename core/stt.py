...
class AbstractSTTEngine(object):
    ...
    def get_instance(cls, vocabulary_name, phrases):
        ...
        if cls.VOCABULARY_TYPE:
            vocabulary = cls.VOCABULARY_TYPE(vocabulary_name, path=aliadapath.config('vocabularies'))
...
class PocketSphinxSTT(AbstractSTTEngine):
    ...
    def __init__(self, vocabulary, hmm_dir="/usr/local/share/pocketsphinx/model/hmm/en_US/hub4wsj_sc_8k", **kwargs):
        """
        ...
        # meaningful error messages if necessary
        ...
        if missing_hmm_files:
            self._logger.warning("hmm_dir '%s' is missing files: %s. Please make sure that you have set the correct "
...
class BaiduSTT(AbstractSTTEngine):
    ...
    def transcribe(self, fp):
        ...
        r = requests.post('http://vop.baidu.com/server_api', data=data, headers={'content-type': 'application/json'})
        try:
            r.raise_for_status()
            text = ''
            if 'result' in r.json():
...
class ALiBaBaSTT(AbstractSTTEngine):
    ...
    def to_md5_base64(self):
        hs = hashlib.md5()
        hs.update(self.body)
        m = hs.digest().encode('base64').strip()
        hs = hashlib.md5()
        hs.update(m)
...
class SnowboySTT(AbstractSTTEngine):
    ...
    Snowboy STT 离线识别引擎（只适用于离线唤醒）
        ...
        snowboy:
            model: '/home/robot/.aliada/snowboy/aliada.pmdl'  # 唤醒词模型
            sensitivity: "0.5"  # 敏感度
        ...
    """
    SLUG = "snowboy-stt"
    ...
    def __init__(self, sensitivity, model, hotword, **kwargs):
        ...
        self.model = model
        self.resource_file = os.path.join(aliadapath.LIB_PATH, 'snowboy/common.res')
        try:
            from .snowboy import snowboydetect
        except Exception as e:
            self._logger.critical(e)
            if 'libf77blas.so' in e.message:
                self._logger.critical("您可能需要安装一个so包加载库：sudo apt-get install libatlas-base-dev")
            return
    ...
    def get_config(cls):
        ...
        if 'model' not in profile:
            profile['model'] = os.path.join(
                aliadapath.LIB_PATH, 'snowboy/aliada.pmdl')
        if 'sensitivity' not in profile:
            profile['sensitivity'] = "0.5"
        profile['hotword'] = config.get('hotword', 'ALIADA')
...
def get_engine_by_slug(slug=None):
    ...
    selected_engines = filter(lambda stt_engine: hasattr(stt_engine, "SLUG") and stt_engine.SLUG == slug, get_engines())
