...
class Mic(object):
    ...
    def __init__(self, speaker, passive_stt_engine, active_stt_engine):
        ...
        passive_stt_engine -- performs STT while Aliada is in passive listen mode
        acive_stt_engine -- performs STT while Aliada is in active listen mode
        """
        self.robot_name = config.get('robot_name_cn', u'Aliada')
        self._logger = logging.getLogger(__name__)
        self.speaker = speaker
        self.wxbot = None
        self.passive_stt_engine = passive_stt_engine
        self.active_stt_engine = active_stt_engine
        self.aliadapath = aliadapath
    ...
    def fetch_threshold(self):
        ...
        RATE = 16000
        CHUNK = 1024
        THRESHOLD_TIME = 1  # number of seconds to allow to establish threshold
    ...
    def passive_listen(self, PERSONA):
        ...
        transcribed = self.passive_stt_engine.transcribe_keyword(''.join(frames))
    ...
    def before_listen_event(self):
        # run plugins before listen
        for plugin in plugin_loader.get_plugins_before_listen():
            continueHandle = False
            try:
                continueHandle = plugin.beforeListen(self, config.get(), self.wxbot)
            except Exception as e:
                self._logger.error("plugin '%s' run error: %s" % (plugin.__name__, e), exc_info=True)
            finally:
                if not continueHandle:
                    break

    def end_listen_event(self):
        # run plugins after listen
        for plugin in plugin_loader.get_plugins_after_listen():
            continueHandle = False
            try:
                continueHandle = plugin.afterListen(self, config.get(), self.wxbot)
            except Exception as e:
                self._logger.error("plugin '%s' run error: %s" % (plugin.__name__, e), exc_info=True)
            finally:
                if not continueHandle:
    ...
    def say(self, phrase, OPTIONS=" -vdefault+m3 -p 40 -s 160 --stdout > say.wav", cache=False):
        ...
        if self.wxbot is not None:
            wechatUser(config.get(), self.wxbot, "%s: %s" % (self.robot_name, phrase), "")

        # in-case calling say() method which have not implement cache feature yet.
    ...
    def play(self, src):
        # play a voice in current thread.
        self.sound.play_block(src)

    def play_no_block(self, src):
        # using a new thread to play a voice
        self.sound.play(src)
