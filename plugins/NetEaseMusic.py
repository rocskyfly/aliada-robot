    ...
    def play(self, report=True):
        ...
        if self.idx < len(self.playlist):
            ...
            try:
                ...
                # Todo: need to be verified: not using mic instance to play music.
                print("==================self.volume=%s, mp3_url=%s=============" % (self.volume, mp3_url))
                p = subprocess.Popen("play -v %f %s" % (self.volume, mp3_url), shell=True, stdout=subprocess.PIPE)
                p.communicate()
                print("p.returncode", p.returncode)
                # if err:
                #     print("==================sThere is an error:", err)
                # else:
                #     print ("==================There is no error:", output)
                self.cond.notify()
