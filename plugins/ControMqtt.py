...
# The actions' file must be /home/robot/.aliada/action.json
...
def get_topic(text):
    ...
    location = home_dir + '/.aliada/action.json'
...
def handle(text, mic, profile, wxbot=None):
    ...
    # get config
    if (SLUG not in profile) or (not profile[SLUG].has_key('host')) or (not profile[SLUG].has_key('port')) or (
            not profile[SLUG].has_key('topic_s')):
        mic.say("主人，配置有误")
        return
