...
def handle(text, mic, profile, wxbot=None):
    if u"帮我" in text:
        ...
        input = mic.active_listen(MUSIC=True)
    while not input:
...
def hass(text, mic, profile):
    ...
    for device in devices:
        ...
        if 'aliada' in attributes.keys():
            aliada = attributes["aliada"]
            if isinstance(aliada, list):
                if text in aliada:
                    try:
                    ...
                    if 'measurement' in locals().keys():
                        ...
                        mic.say(text, cache=True)
                    break
            elif isinstance(aliada, dict):
                if text in aliada.keys():
                    if isinstance(text, bytes):
                        text = text.decode('utf8')
                    try:
                        act = aliada[text]
