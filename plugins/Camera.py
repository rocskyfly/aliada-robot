...
def handle(text, mic, profile, wxbot=None):
    ...
    """
    sys.path.append(mic.aliadapath.LIB_PATH)
    ...
    if profile[SLUG] and 'enable' in profile[SLUG] and \
        ...
        try:
            ...
            mic.say(u"抱歉，照片目录创建失败:%s" % e, cache=True)
            return
        ...
        if sound:
            mic.play(mic.aliadapath.data('audio', 'camera.wav'))
        # send to user
        if send_to_user:
            target = u'邮箱'
...
def isValid(text):
    """
        ...
        text -- user-input, typically transcribed speech
    """
    return any(word in text for word in [u"拍照", u"拍张照"])
