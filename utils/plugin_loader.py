...
import logging
import pkgutil
from . import aliadapath
from . import config

_logger = logging.getLogger(__name__)
_has_init = False  # plugins initialization flag.
_plugins_query = []  # plugins run at query
_plugins_before_listen = []  # plugins run at before listen
_plugins_after_listen = []  # plugins run at after listen
# _third_party_exclude_plugins = {'netease_music'}
_third_party_exclude_plugins = {}
...
def init_plugins():
    ...
    global _has_init, _plugins_query, _plugins_before_listen, _plugins_after_listen
    locations = [
        aliadapath.PLUGIN_PATH,
        aliadapath.CUSTOM_PATH
    ]
    nameSet = set()

    _logger.debug("Looking for plugins in: %s", ', '.join(["'%s'" % location for location in locations]))
    ...
    for finder, name, ispkg in pkgutil.walk_packages(locations):
        try:
            ...
            _logger.warning("Skipped plugin '%s' due to an error: %s" % (name, e), exc_info=True)
            continue
        ...
        if mod.SLUG in nameSet:
            _logger.warning("plugin '%s' SLUG(%s) has repetition" % (name, mod.SLUG))
            continue
        ...
        if hasattr(mod, 'WORDS'):
            if not hasattr(mod, 'handle') or not hasattr(mod, 'isValid'):
                ...
                _logger.debug("Found query plugin '%s' with words: %r" % (name, mod.WORDS))
...
def check_thirdparty_exclude(mod):
    if mod.SLUG in _third_party_exclude_plugins:
        return True
    return False
