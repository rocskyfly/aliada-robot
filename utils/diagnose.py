...
def check_network_connection(server="www.baidu.com"):
    """
    Checks if Aliada can connect a network server.

    Arguments:
        server -- (optional) the server to connect with (Default: www.baidu.com")
...
def run():
    ...
    logger.info("Starting Aliada diagnostic at %s" % time.strftime("%c"))
    ...
    for fname in [os.path.join(aliadapath.APP_PATH, os.pardir,
                               "phonetisaurus",
                               "g014b2b.fst")]:
