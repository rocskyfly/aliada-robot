...
class TulingRobot(AbstractRobot):
    ...
    def chat(self, texts):
        ...
        try:
            ...
            if len(result) > max_length and self.profile['read_long_content'] is not None and \
            ...
            if result.endswith('?') or result.endswith(u'？') or u'告诉我' in result or u'请回答' in result:
                self.mic.skip_passive = True

        except Exception as e:
            self._logger.critical("Tuling robot failed to response for %r:%s" % (msg, e), exc_info=True)
            self.mic.say("抱歉, 我的大脑短路了, 请稍后再试试.", cache=True)
...
class Emotibot(AbstractRobot):
    ...
    def chat(self, texts):
        ...
        try:
            ...
            self._logger.critical("Emotibot failed to responsed for %r:%s" % (msg, e), exc_info=True)
            self.mic.say("抱歉, 我的大脑短路了,请稍后再试试.", cache=True)
...
def get_robot_by_slug(slug):
    ...
    selected_robots = filter(lambda chat_robot: hasattr(chat_robot, "SLUG") and chat_robot.SLUG == slug, get_robots())
    if len(selected_robots) == 0:
        raise ValueError("No robot found for slug '%s'" % slug)
    else:
        if len(selected_robots) > 1:
            print("WARNING: Multiple robots found for slug '%s'. This is most certainly a bug." % slug)
