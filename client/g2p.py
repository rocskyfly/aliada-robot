...
class PhonetisaurusG2P(object):
    ...
    @classmethod
    def get_config(cls):
        conf = {'fst_model': os.path.join(aliadapath.APP_PATH, os.pardir, 'phonetisaurus', 'g014b2b.fst')}
    ...
    @classmethod
    def execute(cls, fst_model, feed_in, is_file=False, nbest=None):
        ...
        try:
            ...
            logger.error("Error occurred while executing command '%s'", ' '.join(cmd), exc_info=True)
            raise
    ...
    def _translate_words(self, words):
        ...
        output = self.execute(self.fst_model, tmp_fname, is_file=True, nbest=self.nbest)
...
if __name__ == "__main__":
    ...
    parser.add_argument('fst_model', action='store', help='Path to the FST Model')
    ...
    g2pconv = PhonetisaurusG2P(paras.fst_model, nbest=3)
    out = g2pconv.translate(ws)
