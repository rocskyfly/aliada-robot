...
def sendEmail(SUBJECT, BODY, ATTACH_LIST, TO, FROM, SENDER,
    ...
    for attach in ATTACH_LIST:
        try:
            ...
            _logger.error(u'附件 %s 发送失败！:%s' % (attach, e))
            continue
...
def wechatUser(profile, wxbot, SUBJECT="", BODY="", ATTACH_LIST=list(), IMAGE_LIST=list()):
    _logger = logging.getLogger(__name__)
...
def sendToUser(profile, wxbot, SUBJECT="", BODY="", ATTACH_LIST=list(), IMAGE_LIST=list()):
    send_type = 0
    if wxbot is not None and wxbot.my_account != {} \
