...
try:
    ...
    logging.getLogger(__name__).error("Error importing CMUCLMTK plugin.PocketsphinxVocabulary will not work "
...
class AbstractVocabulary(object):
    ...
    def __init__(self, name='default', path='.'):
        ...
        """
        self.name = name
        # self.PATH_PREFIX = 'pocketsphinx-vocabulary'
    ...
    def compile(self, phrases, force=False):
        ...
        if not force and self.compiled_revision == revision:
            self._logger.debug('Compilation not necessary, compiled version matches phrases.')
            return revision
...
class PocketsphinxVocabulary(AbstractVocabulary):
    ...
    def _compile_dictionary(self, words, output_file):
        ...
        Arguments:
            ...
            output_file -- the path of the file this dictionary will be written to.
        """
...
class JuliusVocabulary(AbstractVocabulary):
    ...
    @staticmethod
    def _get_grammar(phrases):
        return {'S': [['NS_B', 'WORD_LOOP', 'NS_E']],
                'WORD_LOOP': [['WORD_LOOP', 'WORD'], ['WORD']]}

    @staticmethod
    def _get_word_defs(lexicon, phrases):
    ...
    def _compile_vocabulary(self, phrases):
        ...
        lexicon_file = aliadapath.data('julius-stt', 'VoxForge.tgz')
        lexicon_archive_member = 'VoxForge/VoxForgeDict'
        profile_path = aliadapath.config('profile.yml')
...
def get_keyword_phrases():
    """
    Gets the keyword phrases from the keywords file in the Aliada data dir.
    ...
    with open(aliadapath.data('keyword_phrases'), mode="r") as f:
        for line in f:
            phrase = line.strip()
            if phrase:
...
if __name__ == '__main__':
    ...
    print("Plugin phrases:    %r" % all_phrases)
    ...
    for subclass in AbstractVocabulary.__subclasses__():
        if hasattr(subclass, 'PATH_PREFIX'):
            ...
            print("Matches phrases:   %r" % vocab.matches_phrases(all_phrases))
            if not vocab.is_compiled or not vocab.matches_phrases(all_phrases):
                print("Compiling...")
                vocab.compile(all_phrases)
                print("")
                print("Vocabulary in:     %s" % vocab.path)
                print("Revision file:     %s" % vocab.revision_file)
                print("Compiled revision: %s" % vocab.compiled_revision)
